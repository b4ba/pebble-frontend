...
class JoinedElectionList extends StatelessWidget {
  ...
  const JoinedElectionList({Key? key, required this.user}) : super(key: key);
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider<ElectionBloc>(
            create: (context) =>
                ElectionBloc()..add(LoadJoinedElectionEvent(user: user))),
      ],
      child: WillPopScope(
        onWillPop: null,
        child: Scaffold(
          ...
          body: Column(
            ...
            children: [
              // SearchBar(),
              // Add the content for just ended elections
              // (ElectionJustEndedBloc) if needed
              Expanded(
                child: Column(
                  children: [
                    Padding(
                    ...
                    BlocBuilder<ElectionBloc, ElectionState>(
...
//   @override
//   Widget build(BuildContext context) {
//     return BlocProvider<ElectionBloc>(
...
//                     BlocBuilder<ElectionBloc, ElectionState>(
//                       builder: (context, state) {
//                         if (state is ElectionInitial) {
//                           // Load joined elections on initial state
//                           BlocProvider.of<ElectionBloc>(context)
//                               .add(LoadJoinedElectionEvent(user: user));
//                           return const CircularProgressIndicator(
//                             color: Colors.blue,
//                           );
//                         } else if (state is ElectionLoaded) {
//                           // Display the list of joined elections
//                           return Expanded(
//                             child: ListView.builder(
//                               padding: const EdgeInsets.only(bottom: 100),
//                               itemCount: state.elections.length,
//                               itemBuilder: (_, index) {
//                                 final election = state.elections[index];
//                                 // Use ElectionCard or any custom widget to display the election details
//                                 return ElectionCard(
//                                   id: election.id,
//                                   electionTitle: election.title,
//                                   electionDescription: election.description,
//                                   electionOrganization: election.organization,
//                                   status: election
//                                       .status, // Replace with the actual status property from the Election model
//                                   userId: user.id,
//                                 );
//                               },
//                             ),
