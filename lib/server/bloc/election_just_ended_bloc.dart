...
class ElectionJustEndedBloc
    extends Bloc<ElectionJustEndedEvent, ElectionJustEndedState> {
  ElectionJustEndedBloc() : super(ElectionJustEndedInitial()) {
    on<LoadElectionJustEnded>((event, emit) async {
      ...
      ElectionStatusEnum status =
          Voter.voters[1].joinedElections[event.elections[0]]!;
      emit(ElectionJustEndedLoaded(
          election: event.elections[0], status: status));
    });
  }
}
